...
function calculateDynamicState(speedKmh, deceleration, radius, muMax) {
  ...
  // Минимальный безопасный радиус
  const minSafeRadius = (v_ms * v_ms) / (muMax * CAR.g);
  ...
  // Требуемая продольная сила со стороны передней оси ТС для достижения замедления:
  const Fx_front_required = CAR.m * deceleration * CAR.brakeDistFront;

  // Без CSC давление делится поровну (50/50):
  const Fx_req_per_wheel = Fx_front_required / 2;

  // --- ФИЗИЧЕСКИ КОРРЕКТНЫЙ РАСЧЕТ СКОЛЬЖЕНИЯ ШИНЫ ---
  // Моделирует плавный переход рабочей точки в зону глубокого скольжения при перегрузке моментом
  const calculateSlipFromForce = (Fx_req, Fx_max, s_crit) => {
    if (Fx_max <= 0 || s_crit <= 0) return 100; // Колесо разгружено в ноль -> полный буксование/юз
    
    const forceRatio = Fx_req / Fx_max;
    
    if (forceRatio <= 0.95) {
      // Стабильная зона качения: скольжение нарастает плавно и нелинейно (квадратично)
      // При малых силах дает реалистичные 2-4% скольжения
      return s_crit * Math.pow(forceRatio, 1.8);
    } else {
      // Зона перехода за пик сцепления (нестабильная фаза торможения)
      // Скольжение нарастает прогрессивно в зависимости от величины избыточного момента
      const excess = forceRatio - 0.95;
      
      // Экспоненциальное скольжение моделирует динамику падения угловой скорости заблокированного колеса
  ...
  // Вердикт о блокировке без систем активного распределения
  // Блокировка фиксируется при уходе в глубокое нестабильное скольжение (например, выше критического порога)
  ...
  // CSC срезает избыток силы на внутреннем колесе до безопасного уровня
  ...
  // Процент снижения давления во внутреннем контуре системой CSC
...
export default function App() {
  ...
  const [speed, setSpeed] = useState(80);       // км/ч (установили по умолчанию 80 согласно запросу)
  const [deceleration, setDeceleration] = useState(2.8); // м/с² (установили по умолчанию 2.8)
  const [radius, setRadius] = useState(105);     // метров (установили по умолчанию 105)
  const [muMax, setMuMax] = useState(0.65);     // Коэффициент сцепления покрытия (установили по умолчанию 0.65)
  ...
  const setOptimalRadius = () => {
    ...
    const safeR = Math.ceil((v_ms * v_ms) / (muMax * CAR.g) * 1.15);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 bg-slate-800/80 backdrop-blur border border-slate-700 p-6 rounded-2xl space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                Установить безопасный радиус (~{Math.ceil(calculations.minSafeRadius * 1.15)}м)
              </button>
            </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {/* Линия требуемой тормозной силы от педали (одинаковая нагрузка на колеса) */}
                        ...
                        <g>
                          <circle 
                            cx={50 + Math.min(50, calculations.s_actual_VF) * 8.2} 
                            cy={Math.max(20, Math.min(170, yBrakeReq))} 
                            r="6" 
                            fill="#10b981" 
                            stroke="#fff" 
                            strokeWidth="2" 
                          />
                          <text 
                            x={50 + Math.min(50, calculations.s_actual_VF) * 8.2} 
                            y={Math.max(20, Math.min(170, yBrakeReq)) - 10} 
                            fill="#10b981" 
                            fontSize="10" 
                            fontWeight="bold" 
                            textAnchor="middle"
                        ...
                        <g>
                          <circle 
                            cx={50 + Math.min(50, calculations.s_actual_VN) * 8.2} 
                            cy={calculations.isLocked_VN ? 170 : Math.max(20, Math.min(170, yBrakeReq))} 
                            r="6" 
                            fill={calculations.isLocked_VN ? "#ef4444" : "#f59e0b"} 
                            stroke="#fff" 
                            strokeWidth="2" 
                          />
                          <text 
                            x={50 + Math.min(50, calculations.s_actual_VN) * 8.2} 
                            y={(calculations.isLocked_VN ? 170 : Math.max(20, Math.min(170, yBrakeReq))) - 10} 
                            fill={calculations.isLocked_VN ? "#ef4444" : "#f59e0b"} 
                            fontSize="10" 
                            fontWeight="bold" 
                            textAnchor="middle"
                          >
                            {calculations.isLocked_VN 
                              ? `VN: ${calculations.s_actual_VN.toFixed(1)}% (Срыв)` 
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="bg-slate-900/60 text-slate-400 border-b border-slate-700 text-xs uppercase">
                      ...
                      <th className="py-3 px-4">Вмешательство CSC</th>
                    </tr>
                  </thead>
                          ...
                          <td className="py-3 px-4">
                            {hasCscReduction ? (
                              <span className="text-blue-400 font-bold">
                                Сброс давления на {row.cscReduction.toFixed(0)}%
                              </span>
                            ) : (
